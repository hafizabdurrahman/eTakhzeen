...
function CategoriesView({ onSelect }) {
    ...
    if (loading) return <p className="text-sm text-stone-500 dark:text-stone-400">Loading categories...</p>;
    if (error) return <p className="text-sm text-red-600 dark:text-red-400">{error}</p>;
    if (entries.length === 0) return <p className="text-sm text-stone-500 dark:text-stone-400">No categories yet.</p>;
    ...
    return (
        <div>
            <h2 className="mb-1 text-xl font-semibold text-stone-900 dark:text-stone-100">Categories</h2>
            <p className="mb-5 text-sm text-stone-500 dark:text-stone-400">
                {entries.length} categor{entries.length === 1 ? 'y' : 'ies'} across your catalog.
            </p>
